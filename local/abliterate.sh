...
if command -v abliterix >/dev/null 2>&1 || python3 -c "import abliterix" 2>/dev/null; then
  ...
  abliterix --model "$SRC" --out "$OUT" || \
    python3 -m abliterix --model "$SRC" --out "$OUT"
else
  ...
  mkdir -p "$OFFLOAD"
  python3 abliterate.py --model "$SRC" --out "$OUT" --offload_folder "$OFFLOAD"
fi

echo
echo "== abliterated -> $OUT"
echo "VERIFY it complies before quantizing:"
echo "  python3 -c \"from transformers import *; import torch; \\"
